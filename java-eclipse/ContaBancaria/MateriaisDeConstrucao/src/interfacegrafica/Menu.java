...
public class Menu {
	...
	public void menuOpcoes() {
		...
		while(true) {
			int opcao = Integer.parseInt(JOptionPane.showInputDialog("Escolha uma das opções abaixo:\n"
					...
					+ "2 - Adicionar Produto já Cadastrado\n"
			...
			if(estoque.getListaDeProdutos()==null) {
				...
				for (Produto prod : estoque.getListaDeProdutos()) {
					if(codigo==prod.getCodigo()) {
						verifica=2;
						JOptionPane.showMessageDialog(null, "Já exite produto usando esse codigo (Produto: "+prod.getNome()+")","Codigo já esta sendo usado!",JOptionPane.WARNING_MESSAGE);
						break;
					}else
						verifica=1;
				}
		}while(verifica==2);
		double valor = Double.parseDouble(JOptionPane.showInputDialog("Insira o preço do(a) "+nome+": "));
		int quantidade = Integer.parseInt(JOptionPane.showInputDialog("Quantidade do produto "+nome+": "));
		String descricao = JOptionPane.showInputDialog("Descrição do produto "+nome+": ");
	...
	private void VisualizarEstoque(Estoque estoque) {
		...
		if((estoque.getListaDeProdutos()==null)||(estoque.getListaDeProdutos().isEmpty())) 
			tudo = "Não a nada disponivel no estoque.";
		else {
			for(Produto prod : estoque.getListaDeProdutos()) {
				tudo += "Nome do produto: "+prod.getNome()+"   Codigo:"+prod.getCodigo()+"   Valor: "+prod.getValor()+
						"   Quantidade: "+prod.getQuant()+"   Descrição: "+prod.getDescricao()+"\n";
			}
		}
	...
	private void visualizarListDeProdutos(Estoque estoque) {
		...
		int verifica = 0;
		if((estoque.getListaDeProdutos()==null)||(estoque.getListaDeProdutos().isEmpty())) { 
			tudo = "Não a produtos cadastrados!";
			verifica = 2;
		}else {
			for(Produto prod : estoque.getListaDeProdutos()) {
		...
		if(verifica == 0) 
			...
			JOptionPane.showMessageDialog(null, tudo,"Não a produtos!",JOptionPane.WARNING_MESSAGE);
	}

	private Produto vendaProduto(Estoque estoque) {
		Produto produto = new Produto();
		if((estoque.getListaDeProdutos()==null)||(estoque.getListaDeProdutos().isEmpty())) {
			JOptionPane.showMessageDialog(null, "Não a nem um produto cadastrado!","Não a produtos!",JOptionPane.WARNING_MESSAGE);
			produto.setQuant(-2);
		}else {
			...
			do {
				...
				for(Produto prod : estoque.getListaDeProdutos()) {
					...
					if(codigo==prod.getCodigo()) {
						...
						verifica = JOptionPane.showConfirmDialog(null,"Esse é o produto que deseja vender: "+prod.getNome()+"?","Verificação!",JOptionPane.YES_NO_OPTION);
						...
						if((verifica==0)&&(prod.getQuant()!=0)) {
							...
							do {
								...
								if(soma<0)
									...
									JOptionPane.showMessageDialog(null, "Não a produtos suficientes no estoque para realizar a venda!","Não a produtos!",JOptionPane.WARNING_MESSAGE);
								else if(quantidade==0) 
									JOptionPane.showMessageDialog(null, "Não é possivel vender \"0\" produtos realizar a venda!","Não a produtos!",JOptionPane.WARNING_MESSAGE);
								else {
									verifica=3;
									prod.setQuant(soma);
									valor = prod.getValor();
							...
							JOptionPane.showMessageDialog(null, "Não a mais esse produto no estoque para se realizar a venda!","Não a produtos!",JOptionPane.WARNING_MESSAGE);
							produto.setQuant(-2);
						}else if(verifica==1) {
	...
	private void visualizarNotas(NotasFiscais notasF,Estoque estoque) {
		...
		if((notasF.getListaDeNotas()==null)||(notasF.getListaDeNotas().isEmpty())) {
			JOptionPane.showMessageDialog(null, "Não ouve nem uma venda então não ouve geração de nota fiscal!","Não a Notas Fiscais!",JOptionPane.WARNING_MESSAGE);
		}else {
	...
	private void valorTotalVendas(NotasFiscais notasF) {
		if((notasF.getListaDeNotas()==null)||(notasF.getListaDeNotas().isEmpty()))
			JOptionPane.showMessageDialog(null, "Não ouve nem uma geração de nota fiscal!","Não Ouve Vendas!",JOptionPane.WARNING_MESSAGE);
		else {
			int valorT = 0;
			for(Produto produto: notasF.getListaDeNotas()) {
				valorT+= produto.getValor();
			}
			JOptionPane.showMessageDialog(null, "O valor todal de vendas é: "+valorT);
		}
	}
